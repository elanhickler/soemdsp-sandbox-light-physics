...
This first shell consumes the generated bound WAV resync artifact packet from the sibling `soemdsp` repository. It displays manifest status, boundary flags, phase reports with time ranges, parameter resync values and change ratios, artifact links, a read-only waveform with phase seek controls, and the generated WAV through a browser-native audio control.
